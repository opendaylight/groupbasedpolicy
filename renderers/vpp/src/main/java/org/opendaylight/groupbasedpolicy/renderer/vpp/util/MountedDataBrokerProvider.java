...
public class MountedDataBrokerProvider {
    ...
    public DataBroker resolveDataBrokerForMountPoint(@Nonnull InstanceIdentifier<Node> iidToMountPoint) {
        try {
            if (VbdNetconfTransaction.NODE_DATA_BROKER_MAP.get(iidToMountPoint) != null) {
                return VbdNetconfTransaction.NODE_DATA_BROKER_MAP.get(iidToMountPoint).getKey();
            }
