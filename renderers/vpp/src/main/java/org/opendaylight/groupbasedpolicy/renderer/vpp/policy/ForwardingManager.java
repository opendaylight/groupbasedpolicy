...
public final class ForwardingManager {
    ...
    private boolean isBviForEndpoint(AddressEndpointWithLocation rEp) {
        LOG.trace("Testing BVI for adressEP {}", rEp);
        java.util.Optional<ChildEndpoint> childEndpointOpt = rEp.getChildEndpoint().stream().findFirst();
        VppEndpointKey vppEndpointKey;
        if(!childEndpointOpt.isPresent()) {
            vppEndpointKey =
        ...
        if (vppEndpointOptional.isPresent()) {
            LOG.trace("Testing VPP endpoint: {} for BVI status. VppEP type: {}", vppEndpointOptional.get(),
                vppEndpointOptional.get().getInterfaceTypeChoice());
            ...
            LOG.trace("Cannot distinguish if endpoint is BVI for VPP endpoint with key: {}.", vppEndpointKey);
        }
